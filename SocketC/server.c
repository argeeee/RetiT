...
int main(int argc, char **argv) {
	...
	if (setsockopt(listen_sd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)) < 0) {
		perror("set listening socket options");
		exit(1);
	}
	printf("Server: set listening socket options OK\n");

	if (bind(listen_sd, (struct sockaddr *)&servaddr, sizeof(servaddr)) < 0) {
		perror("bind listening socket");
		exit(1);
	}
	printf("Server: bind listening socket OK\n");
	...
	for (;;)
	{
		...
		if (fork() == 0) { // child
			...
			if (host == NULL) {
				...
				printf("Server (child): host client is %s \n", host->h_name);
			}

			// LOGIC HERE
			if (nread = read(conn_sd, &line, sizeof(int)) <= 0) {
				sprintf(err, "(PID %d) cannot read line number", getpid());
				perror(err);
				exit(EXIT_FAILURE);
			}
			printf("Server (child), line: %d\n", line);
			...
			while ((nread = read(conn_sd, &ccar, sizeof(char))) > 0) {
				...
				if (ccar == '\n') { // finita una linea
					if (lineCount != line) { // se è la linea da saltare, non trasmetto nemmeno il newline
