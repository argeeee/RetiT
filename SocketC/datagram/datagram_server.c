...
int main(int argc, char **argv) {
	...
	if (argc != 2) {
		...
		while (argv[1][nread] != '\0') {
			if ((argv[1][nread] < '0') || (argv[1][nread] > '9')) {
				printf("readed port is not an integer\n");
				printf("Error: %s port\n", argv[0]);
				exit(2);
			}
			nread++;
	...
	printf("Server: soxket created, sd=%d\n", sd);
	...
	for (;;) {
		...
		printf("Requested operetion for file: %s\n", nomeFile);
		clienthost = gethostbyaddr((char *)&cliaddr.sin_addr, sizeof(cliaddr.sin_addr), AF_INET);
		if (clienthost == NULL)
			printf("client host information not found\n");
		else
			printf("Requesto from: %s %i\n", clienthost->h_name, (unsigned)ntohs(cliaddr.sin_port));
		...
		if ((fd = open(nomeFile, O_RDONLY)) < 0) {
			...
			while ((nread = read(fd, &read_char, sizeof(char))) != 0) {
				if (nread < 0) {
					sprintf(err, "(PID %d) cannot read from file", getpid());
					perror(err);
					exit(0);
				} else {
